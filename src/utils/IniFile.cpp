...
bool IniFile::write(const string &key, long value) const 
{
	char tmp[64]={0};
	sprintf(tmp,"%u",value);
	return write(key,tmp);
...
bool IniFile::readStr(const string &key,string &value) const
{
	char buf[4096]={0};
	if(!read_profile_string(m_section.c_str(),key.c_str(),buf,sizeof(buf),value.c_str(),m_fileName.c_str()))
	{
		return false;
	}
	else
...
bool IniFile::readInt(const string &key, int &value) const
{
	return read_profile_int(m_section.c_str(),key.c_str(),value,m_fileName.c_str());
}

bool IniFile::readLong(const string &key, long &value) const
{
	return read_profile_long(m_section.c_str(),key.c_str(),value,m_fileName.c_str());
}
					...
					&& !end_of_string(p[i]) ) {
						...
						if(';' != p[j]) //skip over comment
						{
							while(j < i && p[j]!='=') {
								j++;
								if('=' == p[j]) {
									char chTempKey[256]={0};
									strncpy(chTempKey,p+newline_start,j-newline_start);
									/*
									...
									*/
									...
									while(chTempKey[strlen(chTempKey)-1]=='\r'||chTempKey[strlen(chTempKey)-1]=='\n'||chTempKey[strlen(chTempKey)-1]==' ')
									{
										chTempKey[strlen(chTempKey)-1]='\0';
									}
									if(strcmp(key,chTempKey)==0)
									{
										//find the key ok
										*key_s = newline_start;
										*key_e = j-1;
...
*/
bool IniFile::read_profile_int( const char *section, const char *key,int &ivalue, 
							   const char *file)
{
	char value[32] = {0};
...
*@return profile int value,if read fail, return default value
*/
bool IniFile::read_profile_long( const char *section, const char *key,long &ivalue, 
								const char *file)
{
	char value[32] = {0};
	if(!read_profile_string(section,key,value, sizeof(value),NULL,file))
